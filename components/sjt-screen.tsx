...
export function SJTScreen({ userData: _userData, onNext, onResults }: SJTScreenProps) {
  ...
  const handleNext = () => {
    ...
    if (currentScenario < sjtScenarios.length - 1) {
      ...
      const results = Object.entries(answers).map(([scenarioIndex, answerId]) => {
        const scenario = sjtScenarios[parseInt(scenarioIndex)]
