...
export function MiniDiscScreen({ userData: _userData, onNext, onResults }: MiniDiscScreenProps) {
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < discQuestions.length - 1) {
      ...
      const results = {
        D: Math.round((discScores.D / total) * 100),
        I: Math.round((discScores.I / total) * 100),
        S: Math.round((discScores.S / total) * 100),
        C: Math.round((discScores.C / total) * 100)
      }
